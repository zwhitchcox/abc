...
export async function loader({ request }: LoaderFunctionArgs) {
    ...
    const topicsWithCounts = await Promise.all(
        topics.map(async (topic) => {
            ...
            if (await fs.pathExists(topicDir)) {
                // Approximate image count from fs
                try {
    ...
    return json({ topics: topicsWithCounts })
}
...
export async function action({ request }: ActionFunctionArgs) {
    ...
    if (intent === 'addTopic') {
        ...
        try {
            await prisma.flashcardTopic.create({
                data: {
                    name: topicName.trim(),
                    slug
                }
    ...
    if (intent === 'deleteTopic') {
        ...
        const topic = await prisma.flashcardTopic.findUnique({ where: { id: topicId } })
        if (topic) {
            await prisma.flashcardTopic.delete({ where: { id: topicId } })
        ...
        <div className="container mx-auto p-8 max-w-4xl">
            <div className="flex justify-between items-center mb-6">
                <div>
                    ...
                    <p className="text-muted-foreground text-sm">Manage topics and items</p>
                </div>
            ...
            <div className="grid gap-4">
                {topics.length === 0 ? (
                    ...
                    topics.map(topic => (
                        <div key={topic.id} className="flex items-center justify-between p-4 border rounded-lg bg-card shadow-sm hover:shadow-md transition-shadow">
                            <Link to={`/admin/flashcards/${topic.slug}`} className="flex-1 flex items-center gap-4">
                            ...
                            <div className="flex items-center gap-2">
                                <Button asChild variant="ghost" size="sm">
                                    <Link to={`/admin/flashcards/${topic.slug}`}>
