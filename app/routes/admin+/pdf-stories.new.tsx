...
export async function loader({ request }: LoaderFunctionArgs) {
  ...
  return json({ serverAiEnabled: isServerAiEnabled() });
}

export async function action({ request }: ActionFunctionArgs) {
  await requireUserWithRole(request, "admin");
  if (!isServerAiEnabled()) {
    return json({ error: serverAiDisabledMessage() }, { status: 403 });
  }
...
export default function NewPdfStory() {
  const { serverAiEnabled } = useLoaderData<typeof loader>();
    ...
    <div className="container mx-auto p-8 max-w-md">
      ...
      {!serverAiEnabled ? (
        <div className="mb-6 rounded-lg border border-amber-200 bg-amber-50 p-4 text-sm text-amber-900">
          Server-side AI generation is disabled. Generate picture books locally,
          import the generated assets into the content database, then copy the
      ...
      >
          ...
          onDrop={(e) => {
            ...
            if (files && files.length > 0) {
              ...
              if (input) {
                ...
                if (label)
                  label.innerText = count > 0 ? `${count} files selected` : "";
              }
            }
          }}
        >
          ...
          <p
            id="file-count-label"
            className="mt-4 text-sm font-medium text-blue-600 h-5"
          ></p>
        </div>
        <button
          type="submit"
          disabled={
            !serverAiEnabled || isUploading || navigation.state === "submitting"
          }
        ...
        <p className="text-xs text-gray-500 mt-2 text-center">
          Processing happens in the background and may take several minutes per
          file. Stories will appear in the library automatically as pages are
          processed.
        </p>
      </Form>
    </div>
