        ...
        for (const item of topic.items) {
          ...
          try {
            ...
            for (const image of validImages) {
              ...
              imageCount += 1;
              thumbnail ??= `/images/${topic.slug}/${item.slug}/${image}`;
            }
          } catch {}
        }
      } catch {}
...
export default function FlashcardsIndex() {
  ...
  return (
    <div className="min-h-screen bg-stone-950 px-4 py-8 text-white">
              ...
              <div className="aspect-video relative flex items-center justify-center">
                <div className="flex -space-x-4">
                  {topics.slice(0, 3).map((t, i) => (
                    <div
                      key={t.name}
                      className={`w-16 h-16 rounded-full border-4 border-white dark:border-stone-900 overflow-hidden shadow-lg z-${3 - i}`}
                    >
                      {t.thumbnail ? (
                        <img
                          src={t.thumbnail}
