...
async function downloadImagesForItem(
  ...
  const existingImages = fs.readdirSync(itemDir)
  ...
  console.log(`     Already has: ${existingImages.length} | Need: ${needed} more`)
  ...
  try {
    ...
    for (let i = 0; i < imageData.length && downloaded < imagesPerItem; i++) {
      ...
      try {
        ...
        // Show progress
        const downloadProgress = stats.totalNeeded > 0 
          ? Math.round((stats.downloadedCount / stats.totalNeeded) * 100)
          : 100
        
        console.log(`    ✓ Downloaded: ${filename} | Progress: ${stats.downloadedCount}/${stats.totalNeeded} images (${downloadProgress}%)`)
      } catch (error) {
...
async function downloadAllImages() {
  ...
  for (const topic of config.topics) {
    ...
    for (const item of topic.items) {
      ...
      if (fs.existsSync(itemDir)) {
        const existing = fs.readdirSync(itemDir)
  ...
  try {
    for (const topic of config.topics) {
      ...
      try {
        let itemNumber = 0
        for (let topicIndex = 0; topicIndex < config.topics.indexOf(topic); topicIndex++) {
        ...
        for (let i = 0; i < topic.items.length; i++) {
          ...
          itemNumber++
          await downloadImagesForItem(page, topic.name, item, topicDir, imagesPerItem, itemNumber, totalItems, stats)
