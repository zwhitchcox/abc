...
async function storeProcessedFile(filePath: string) {
...
async function storeProcessedDirectory(dir: string) {
  if (!(await fs.pathExists(dir))) return;
  const entries = await fs.readdir(dir, { withFileTypes: true });
  for (const entry of entries) {
    if (entry.name === ".DS_Store" || entry.name.startsWith("._")) continue;
    const entryPath = path.join(dir, entry.name);
    if (entry.isDirectory()) await storeProcessedDirectory(entryPath);
    else if (entry.isFile()) await storeProcessedFile(entryPath);
  }
}
...
async function main() {
  ...
  if (!pdfPath || pdfPath.startsWith("--")) {
    console.error(
      "Usage: npm run process-pdf <path-to-pdf> [--page <number|range>]",
    );
    process.exit(1);
  }
  ...
  if (hasImages) {
    ...
    try {
      ...
      console.error(
        'Error converting PDF to images. Make sure "poppler" is installed (brew install poppler).',
      );
      throw e;
    }
  }
  ...
  const updateProgress = () => {
    ...
    if (process.stdout.isTTY) {
      process.stdout.write(
        `\rProgress: [${"=".repeat(Math.floor(percent / 2))}${" ".repeat(50 - Math.floor(percent / 2))}] ${percent}% (${completed}/${total})`,
      );
    } else {
  ...
  async function processPage(imageFile: string) {
    ...
    if (
      existingMarker &&
      (existingMarker.duration > 0 || existingMarker.text === "")
    ) {
      completed++;
      updateProgress();
      return;
    ...
    while (retries < MAX_RETRIES && !success) {
      try {
        ...
        if (extractedText) {
          ...
          const instructions =
            toneAnalysis.choices[0]?.message?.content?.trim() ||
            "Speak in a warm, engaging storytelling voice.";
        ...
        if (e.status === 429) {
          ...
          console.error(
            `  Rate limit hit for page ${pageNum}. Waiting ${waitTime / 1000}s...`,
          );
  ...
  await saveMarkers();
  await storeProcessedDirectory(outputDir);
